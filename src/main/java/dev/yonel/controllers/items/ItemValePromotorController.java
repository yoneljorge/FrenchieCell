...
public class ItemValePromotorController implements Initializable {
    ...
    public ItemValePromotorController() {
    ...
    public void initialize(URL location, ResourceBundle resource) {
        ...
        try {
            ...
            ItemValeController controller = new ItemValeController(stage, vale);
            ...
            /*
             * Para que cuando salga el puntero del mouse fuera del VBox se cierre el popup.
             */
        ...
        checkBoxLiquidar.selectedProperty().addListener((observable, oldValue, newValue) -> {
            if(checkBoxLiquidar.isSelected()){
                ServicePromotoresControllerPromotor.addValeLiquidar(vale);
            }else{
                ServicePromotoresControllerPromotor.removeValeLiquidar(vale);
            }
        });
    }
    ...
    private void showPopup(double x, double y, Stage ownerStage) {
        if (!popup.isShowing()) { // Solo mostrar si no está visible
            ...
            popup.setX(x - 20);
            popup.setY(y);
            popup.show(ownerStage); // Mostrar el popup
        }
